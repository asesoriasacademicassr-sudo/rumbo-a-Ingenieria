...
function renderAll(){renderLiveRooms();renderDashboard();renderStudents();renderClasses();renderPayments();renderTeachers();renderGuardians();renderResourcesAdmin();renderMessagesAdmin();renderPackages();renderNotifications();renderCertificates();renderReports();renderAthenaHistory();athenaSnapshot();populateHorizonSelects();populateFamilySelects();populateTeacherStudents();applySettings()}
save();generateNotifications();
renderAll();function populateLive(){if($('#liveTeacherSelect'))$('#liveTeacherSelect').innerHTML=data.teachers.map(t=>`<option value="${t.id}">${esc(t.name)}</option>`).join('');if($('#liveStudentsSelect'))$('#liveStudentsSelect').innerHTML=data.students.map(s=>`<option value="${s.id}">${esc(s.name)}</option>`).join('')}function renderLiveRooms(){if(!$('#liveRoomsTable'))return;$('#liveRoomsCount').textContent=data.liveRooms.length;$('#liveActiveCount').textContent=data.liveRooms.filter(r=>r.status==='Activa').length;$('#liveAttendanceCount').textContent=data.liveAttendance.length;$('#liveRoomsTable').innerHTML=data.liveRooms.map(r=>`<article class="live-room-card"><div><h3>${esc(r.title)}</h3><p>${esc(r.teacher)} · ${esc(r.date)} ${esc(r.time)} · ${r.duration} min</p><small>${esc(r.status)} · ${(r.participants||[]).length} alumnos</small></div><div class="live-room-actions"><a href="live.html?room=${r.id}" target="_blank">Entrar</a><button data-toggle-live="${r.id}">${r.status==='Activa'?'Finalizar':'Activar'}</button><button data-delete-live="${r.id}">Eliminar</button></div></article>`).join('')||'<div class="empty">No hay salas.</div>'}$('#newLiveRoomBtn').onclick=()=>{populateLive();$('#liveRoomForm').reset();$('#liveRoomDialog').showModal()};$('#liveRoomForm').onsubmit=e=>{e.preventDefault();const f=Object.fromEntries(new FormData(e.target)),t=data.teachers.find(x=>x.id===+f.teacherId),studentIds=[...$('#liveStudentsSelect').selectedOptions].map(o=>+o.value),participants=studentIds.map(id=>data.students.find(s=>s.id===id)?.name).filter(Boolean);data.liveRooms.unshift({...f,id:Date.now(),teacherId:+f.teacherId,teacher:t?.name||'Profesor',studentIds,participants,duration:+f.duration});save();$('#liveRoomDialog').close();renderAll();toast('Sala creada.')};$('#liveRoomsTable').onclick=e=>{const a=e.target.closest('[data-toggle-live]'),d=e.target.closest('[data-delete-live]');if(a){const r=data.liveRooms.find(x=>x.id===+a.dataset.toggleLive);r.status=r.status==='Activa'?'Finalizada':'Activa';save();renderAll()}if(d&&confirm('¿Eliminar sala?')){data.liveRooms=data.liveRooms.filter(x=>x.id!==+d.dataset.deleteLive);save();renderAll()}};
